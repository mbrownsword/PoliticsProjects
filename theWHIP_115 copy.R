...
for (j in 1:length(BillScrips_2_115$HouseBills)){
  if (is.na(BillScrips_2_115$thaturl[BillScrips_2_115$BillsNos==j])==TRUE){
    ...
    if(length(BillScrips_2_115$thaturl[BillScrips_2_115$BillsNos==j])==0){
      ...
      RepName <- NA
      name_state <- NA
      url_bills_amendments <- paste0(strsplit(as.character(BillScrips_2_115$thaturl[BillScrips_2_115$BillsNos==247]),'summary',fixed=TRUE)[[1]][1],"amendments?pageSize=250")
...
for(i in 1:length(amendment_city_2)){
  if(length(amendment_city_2[[i]])==0){
    ...
    if(is.na(amendment_city_2[[i]][1,1])){
      next()
    }else{
...
for(i in 1:length(numbers_amendments)){
  if(length(numbers_amendments[[i]])==0){
    ...
    if(length(numbers_amendments[[i]])==1){
      ...
      keep_track_1 <- BillScrips_2_115$BillsNos[BillScrips_2_115$HouseBillName_2==BillScrips_2_115$HouseBillName_2[BillScrips_2_115$BillsNos==343]]
      keep_track_1 <- na.omit(keep_track_1)
...
BillScrips_2_115$thaturl[BillScrips_2_115$BillsNos==247]
